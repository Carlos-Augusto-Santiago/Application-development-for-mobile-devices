...
public class MainActivity extends AppCompatActivity {
    ...
    public void conectar() {
        ...
        if (activar) {
            ...
            try {
                if (ActivityCompat.checkSelfPermission(this, Manifest.permission.BLUETOOTH_CONNECT) != PackageManager.PERMISSION_GRANTED) {
                    Log.d("BT Permission","Permiso no otorgado 2");
                    return;
                }
                else{
                    btSocket.connect();
            ...
            myConexionBT.start();
            Log.d("BT Connect","Conexion iniciada");

            //Pruebas de lectura y escritura
            myConexionBT.write("t");
            Log.d("BT Write","Datos escritos");
            myConexionBT.run();
            Log.d("BT Read","Datos recibidos");
        }
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    Log.d("BT Data Input",readMessage);
                    // Envia los datos obtenidos hacia el evento via handler
                    bluetoothIn.obtainMessage(handlerState, bytes, -1, readMessage).sendToTarget();
                } catch (IOException e) {
                    Log.d("BT Run error",e.getMessage());
                    break;
                }
            }
        }
